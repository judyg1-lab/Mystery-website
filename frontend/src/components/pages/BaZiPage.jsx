...
const borderGold = '1px solid rgba(255,204,0,0.2)';
...
export default function BaZiPage() {
  ...
  useEffect(() => {
    ...
    const fetchArticles = async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/api/bazi/articles?tabType=${activeTab}`);
        const data = await res.json();
        if (res.ok) setArticles(data);
      } catch (err) {
        console.error('讀取八字文章失敗:', err);
      }
    };
  ...
  const fetchHistoryLogs = useCallback(async () => {
    try {
      const token = localStorage.getItem('mystic_token');
      const res = await fetch(`${API_BASE_URL}/api/history/bazi`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await res.json();
      if (res.ok) setHistoryLogs(data);
    } catch (err) {
      console.error('讀取八字歷史失敗:', err);
    }
  }, []);
  ...
  return (
    <div style={{ ...mainLayout, ...(activeTab === 'drawing' ? baziDrawingBackdrop : null) }}>
      ...
      <nav style={topNavBar}>
        ...
        <div style={navTabsContainer}>
          ...
          {false && ['origins', 'codex', 'drawing'].map((tab) => (
            ...
            >
              {tab === 'origins' ? <div><div>COSMIC PATH</div><div style={subLabel}>八字源流</div></div>
                : tab === 'codex' ? <div><div>ELEMENTS</div><div style={subLabel}>命理秘典</div></div>
                : <div><div>FORECAST</div><div style={subLabel}>命盤紀錄</div></div>}
      ...
      <main style={contentArea}>
          ...
          <div style={flexLayout}>
            ...
            <section style={detailWrapper}>
              <AnimatePresence mode="wait">
                ...
                ) : <div style={emptyHint}>請選擇一篇八字文獻</div>}
              </AnimatePresence>
            </section>
          </div>
        )}
        ...
        {(activeTab === 'drawing' || activeTab === 'history') && (
          <MysticChartTool systemKey="bazi" view={activeTab === 'history' ? 'history' : 'drawing'} targetHistoryId={location.state?.targetHistoryId} resetKey={divinationResetKey} />
        )}
        {false && activeTab === 'drawing' && (
          <>
            {drawingView === 'home' ? (
              <div style={gatewayCenterContainer}>
                <div style={thothLogo}>BAZI</div>
                <h1 style={gatewayTitle}>四柱命盤推演</h1>
                <div style={btnGroup}>
              ...
              <div style={flexLayout}>
                <div style={sidebarWrapper}>
                  ...
                  <div style={searchBox}>
                    <Search size={16} color={ACCENT} />
                    <input placeholder="搜尋紀錄..." style={searchInput} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                  </div>
                  <div style={sidebarList}>
...
const baziDrawingBackdrop = {
  background:
    'linear-gradient(90deg, rgba(10,7,1,0.86), rgba(12,8,1,0.2) 48%, rgba(9,6,1,0.82)), url("/bazi/baziBackground.png") center / cover no-repeat'
};
...
const sidebarWrapper = {
  ...
  padding: '20px', border: '1px solid rgba(255,204,0,0.14)', backdropFilter: 'blur(5px)',
  display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0
};
const detailWrapper = {
  flex: 1, height: '100%', overflowY: 'auto', background: 'rgba(0, 0, 0, 0.22)',
  borderRadius: '8px', padding: '40px 80px', border: '1px solid rgba(255,204,0,0.14)',
  backdropFilter: 'blur(1.5px)', boxShadow: 'inset 0 0 28px rgba(255,204,0,0.08)'
};
const historyReportHeader = { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '18px' };
const historyDetailHeartBtn = { width: 48, height: 48, borderRadius: '50%', border: '1px solid rgba(255,204,0,0.35)', background: 'rgba(255,204,0,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', boxShadow: '0 0 18px rgba(255,204,0,0.16)', flexShrink: 0 };
...
const itemStyle = { display: 'flex', gap: '12px', alignItems: 'center', justifyContent: 'space-between', padding: '15px 20px', borderRadius: '4px', cursor: 'pointer', transition: '0.18s linear', border: '1px solid transparent', borderBottom: '1px solid rgba(255,255,255,0.05)' };
const itemContent = { flex: 1, display: 'flex', flexDirection: 'column', gap: '6px' };
const activeItem = {
  ...itemStyle,
  background: hoverGold,
  border: '1px solid rgba(255,204,0,0.18)',
  boxShadow: 'inset 0 0 18px rgba(255,204,0,0.06), 0 0 14px rgba(255,204,0,0.08)'
};
...
const gatewayTitle = { fontSize: '2.4rem', letterSpacing: '8px', marginTop: '-80px', marginBottom: '60px', textAlign: 'center', textShadow: '0 0 15px rgba(255, 204, 0, 0.4)' };
...
const hideScrollbarCSS = '*::-webkit-scrollbar { display: none; } * { -ms-overflow-style: none; scrollbar-width: none; }';
