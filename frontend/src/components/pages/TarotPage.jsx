...
import { Search, Sparkles, Download, Heart, Clock, Copy } from 'lucide-react';
import { useNavigate, useLocation } from 'react-router-dom';
import { Pencil, Trash2 } from 'lucide-react';
import ProfileIcon from '../ProfileIcon';
import BackBtn from '../backBtn';
import MysticModal from '../MysticModal';
import TarotDrawingSystem from './TarotDrawingSystem';
...
class StarDust {
  ...
  reset() {
    ...
    this.velocity = 0.15 + Math.random() * 0.35;
  }
  update() {
...
const UniverseCanvas = () => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 1000; i++) particles.push(new StarDust(canvas.width));
    const render = () => {
...
const formatTarotMeaningLines = (text = '') => {
  const cleaned = cleanTarotText(text).replace(/\s*✦\s*/g, '\n✦ ').trim();
  return cleaned.split(/\n+/).map(line => line.replace(/\s+/g, ' ').trim()).filter(Boolean);
};

const getCodexIntro = (content = '') => {
  const idx = content.indexOf('No.');
  if (idx === -1) return '';
  return cleanTarotText(content.slice(0, idx));
};

const parseTarotCardEntries = (content = '') => {
  const idx = content.indexOf('No.');
  if (idx === -1) return [];
  return content.slice(idx).split('No.').filter(Boolean)
    .map(chunk => cleanTarotText(chunk)).filter(Boolean)
    .map((chunk, index) => {
...
const isEnglishText = (text = '') =>
  (text.match(/[a-zA-Z]/g) || []).length / Math.max(text.length, 1) > 0.52;
...
const TarotCardVisual = ({ item, entry }) => (
  <div style={{ ...tarotCardShell, maxWidth: '180px' }}>
    <div style={{ ...tarotImageFrame, aspectRatio: '4 / 6', maxHeight: '280px', marginTop: '30px' }}>
      <img
        src={entry?.card?.imageUrl ? getAssetUrl(entry.card.imageUrl) : TAROT_CARD_BACK_URL}
        alt={entry?.card?.title || entry?.name || ''}
        style={{ ...tarotCardImage, objectFit: 'cover', objectPosition: 'center top' }}
      ...
      <header style={thHeader}>
        <div style={{ flex: 1 }}>
          <div style={thKicker}>
            ...
            {soulMaster !== '─' && (
              <><span style={thKickerDot}>·</span><span>主牌：{soulMaster}</span></>
            )}
...
export default function TarotPage() {
  ...
  const [drawingResetKey, setDrawingResetKey] = useState(0); // ← 重置 key
  ...
  // ── History AI Reading state (放在元件內) ────────────────────────────────
  const [isHistoryAiReading, setIsHistoryAiReading] = useState(false);
  ...
  useEffect(() => {
    ...
    if (savedUser) {
      ...
      } catch (err) { console.error('Parse user info failed:', err); }
    }
  }, []);
  ...
  useEffect(() => {
    const fetchTarotCards = async () => {
      ...
      } catch (err) { console.error('Failed to load tarot card images:', err); }
    };
    fetchTarotCards();
  }, []);
  ...
  useEffect(() => {
    ...
    const matched = articles.find(a =>
      a.id === Number(targetId) || (targetTitle && a.title === targetTitle)
    );
    if (matched) {
  ...
  const [modalConfig, setModalConfig] = useState({
    isOpen: false, title: '', message: '', confirmText: '確認', cancelText: '取消',
    type: 'info', mode: null, onConfirm: () => {}
  });
  ...
  const processedHistoryLogs = historyLogs.filter((log) => {
    ...
    return !(/秘典|克勞利|托特之書|THOTH ARCHIVE|No\.\d|The Fool|The Magus/.test(title) ||
  ...
  const selectedTarotHistoryCards = useMemo(
    () => (selectedTarotHistory?.cards || []).map((card) => ({
      ...card,
      card: tarotCardByName.get(normalizeTarotName(card.slug)) ||
            tarotCardByName.get(normalizeTarotName(card.name)) || null
    })),
  ...
  useEffect(() => {
    ...
    if (activeTab === 'history') {
      ...
      const selectedExistsInHistory =
        selectedType === 'history' && processedHistoryLogs.some(log => log.id === selectedItemId);
    ...
    const selectedExistsInCurrentTab =
      selectedType === 'article' && processedArticles.some(item => item.id === selectedItemId);
      ...
      try {
        ...
        if (res.ok && data.favorite) {
          ...
          setHistoryLogs(prev => prev.map(log => log.id === rec.id
            ? { ...log, isFavorite: true, favoriteId: data.favorite.id } : log));
          await fetchUserFavorites();
        }
      } catch (err) { console.error(err); }
    }
  ...
  // ── History AI Reading handlers (正確放在元件內，可存取 selectedItem) ─────
  const handleHistoryAiReading = async () => {
    if (!selectedItem) return;
    setIsHistoryAiReading(true);
    window.setTimeout(() => setIsHistoryAiReading(false), 2000);
  };

  const handleHistoryCopyPrompt = async () => {
    if (!selectedItem) return;
    const prompt = `請以塔羅占卜師身份，用繁體中文詳細解讀這次牌陣：\n標題：${selectedItem.title}\n\n${selectedItem.content}`;
  ...
  const executeRenameHistory = async (rec) => {
    ...
    try {
      ...
      if (res.ok && data.history) {
        ...
        setModalConfig({
          isOpen: true, title: '重新命名失敗',
          message: data.error || '目前無法更新這筆塔羅紀錄名稱。',
          confirmText: '我知道了', cancelText: '', type: 'error', mode: null,
          onConfirm: closeMysticModal
        });
      }
    } catch (err) { console.error('Rename tarot history failed:', err); }
  };
  ...
  const handleRenameHistory = (e, rec) => {
    ...
    setModalConfig({
      isOpen: true, title: '重新命名塔羅紀錄', message: '',
  ...
  const executeDeleteHistory = async (rec) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        setModalConfig({
          isOpen: true, title: '刪除失敗',
          message: data.error || '目前無法刪除這筆塔羅紀錄。',
          confirmText: '我知道了', cancelText: '', type: 'error', mode: null,
          onConfirm: closeMysticModal
        });
      }
    } catch (err) { console.error('Delete tarot history failed:', err); }
  };

  const handleDeleteHistory = (e, rec) => {
    if (e) e.stopPropagation();
    setModalConfig({
      isOpen: true, title: '刪除塔羅紀錄',
  ...
  const handleTabClick = (tab) => {
    ...
    // ← 點擊 drawing tab 時，重置 key 讓元件完全 remount
    if (tab === 'drawing') setDrawingResetKey(k => k + 1);
  };
  ...
  return (
    <div style={mainLayout}>
      ...
      <main style={isShuffleImmersive ? immersiveContentArea : contentArea}>
          ...
          <div style={flexLayout}>
            <div style={sidebarWrapper}>
              ...
              <div style={sidebarList}>
                {filteredData.length === 0 && <div style={emptyHint}>尚無可顯示的塔羅檔案</div>}
                {filteredData.map(item => (
                  <div
                    key={`${activeTab}-${item.id}`}
                    onClick={() => { setSelectedItemId(item.id); setSelectedType('article'); setSelectedTarotEntry(null); }}
                    style={selectedItem?.id === item.id ? activeItem : itemStyle}
                    onMouseEnter={(e) => {
                      ...
                      <div style={archiveBodyPanel}>
                        {readableParagraphs.map((paragraph, index) => (
                          <p key={index} style={index === 0 ? archiveFirstParagraph : archiveParagraph}>
                            {index === 0 ? (
                              <><span style={dropCap}>{paragraph.charAt(0)}</span>{paragraph.slice(1)}</>
                            ) : paragraph}
                          </p>
                        ))}
                      </div>
        ...
        {/* Drawing — key={drawingResetKey} 讓點擊 tab 時完全重置 */}
        {activeTab === 'drawing' && (
          <TarotDrawingSystem
            key={drawingResetKey}
                ...
                {filteredHistoryLogs.map((rec) => (
                  ...
                  >
                    ...
                    <Pencil
                      ...
                      onMouseEnter={(e) => {
                        e.currentTarget.style.fill = '#a2003e';
            ...
            <section style={detailWrapper}>
              <AnimatePresence mode="wait">
                {selectedItem ? (
                  ...
                  <div style={emptyHint}>請點選一筆塔羅歷史紀錄</div>
                )}
              </AnimatePresence>
            </section>
          </div>
...
// ── TarotHistoryReport styles ─────────────────────────────────────────────────
const thReport = {
  position: 'relative', padding: '28px 30px',
  border: '1px solid rgba(212,175,55,0.22)', borderRadius: '8px',
  background: [
    'radial-gradient(circle at 28% 42%, rgba(188,19,254,0.1), transparent 34%)',
    'radial-gradient(circle at 78% 24%, rgba(212,175,55,0.08), transparent 28%)',
    'rgba(4,1,8,0.55)'
  ].join(', '),
...
const thTitle = {
  margin: 0, color: '#f5ead8',
  fontFamily: '"Noto Serif TC", "Microsoft JhengHei", "PingFang TC", Cinzel, serif',
  fontSize: 'clamp(1.6rem, 2.6vw, 3rem)', letterSpacing: '0.08em', lineHeight: 1.2
};
const thHeartBtn = {
...
const thOrnLine = { height: '1px', background: 'linear-gradient(90deg, transparent, rgba(212,175,55,0.45), transparent)', display: 'block' };
const thCardRow = { display: 'flex', justifyContent: 'center', alignItems: 'flex-start', flexWrap: 'wrap', gap: '28px', paddingTop: '8px' };
const thCardBtn = {
...
const thOracle = {
  margin: '4px 0 0', width: '100%', boxSizing: 'border-box',
  padding: '16px 20px', border: '1px solid rgba(212,175,55,0.2)',
  borderRadius: '8px', background: 'rgba(0,0,0,0.22)', color: '#f5d889', textAlign: 'center'
};
...
const thAnalysisImg = { width: '58px', height: '96px', objectFit: 'cover', borderRadius: '4px', border: '1px solid rgba(212,175,55,0.3)' };
const thAnalysisHead = { display: 'flex', alignItems: 'center', gap: '8px', color: '#f5ead8', letterSpacing: '0.1em' };
const thAnalysisNum = {
...
const thActionBtn = {
  ...
  borderRadius: '4px', border: '1px solid rgba(212,175,55,0.32)',
  background: 'rgba(0,0,0,0.38)',
  color: '#f5ead8', fontFamily: 'Cinzel, serif',
  fontSize: '0.85rem', letterSpacing: '0.12em', cursor: 'pointer',
  transition: 'filter 160ms ease, box-shadow 160ms ease'
};
const thActionBtnPrimary = {
  borderColor: 'rgba(188,19,254,0.52)',
  background: 'linear-gradient(90deg, rgba(188,19,254,0.1), rgba(0,0,0,0.38), rgba(188,19,254,0.08))',
  boxShadow: '0 0 18px rgba(188,19,254,0.14), inset 0 0 18px rgba(188,19,254,0.06)'
};
...
const activeItem = { ...itemStyle, background: 'rgba(90,20,120,0.18)', border: '1px solid rgba(188,19,254,0.18)', boxShadow: 'inset 0 0 12px rgba(188,19,254,0.08), 0 0 8px rgba(188,19,254,0.08)', transition: '0.3s' };
...
const mainTitle = { fontFamily: '"Noto Serif TC", "Songti TC", "PMingLiU", "Cinzel", serif', fontSize: 'clamp(1.5rem, 3vw, 3.5rem)', fontWeight: 400, letterSpacing: '2px', margin: 0, lineHeight: 1.18, color: 'rgba(245,238,222,0.92)' };
...
const archiveHeroLayout = { display: 'grid', gridTemplateColumns: '250px minmax(0, 1fr)', gap: '50px', alignItems: 'center', marginBottom: '25px', padding: '16px 6px 0px', borderBottom: '1px solid rgba(212,175,55,0.16)' };
const archiveHeroTextOnly = { display: 'flex', alignItems: 'center', minHeight: '310px', maxWidth: '980px', margin: '0 auto 34px', padding: '50px 20px 40px', borderBottom: '1px solid rgba(212,175,55,0.16)', boxSizing: 'border-box' };
const archiveCardColumn = { display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', position: 'relative', padding: '18px 22px 30px', border: 'none', background: 'transparent', boxShadow: 'none', overflow: 'visible' };
const archiveSpreadFrameImage = { position: 'absolute', inset: '-22px -34px -24px', width: 'calc(100% + 68px)', height: 'calc(100% + 46px)', objectFit: 'fill', pointerEvents: 'none', opacity: 0.62, zIndex: 0, filter: 'drop-shadow(0 0 18px rgba(188,19,254,0.22)) drop-shadow(0 0 10px rgba(212,175,55,0.18))' };
const archiveCardInfo = { marginTop: '14px', textAlign: 'center', letterSpacing: '3px', lineHeight: 1.5, position: 'relative', zIndex: 2 };
const archiveCardCategory = { color: '#bc13fe', fontSize: '0.7rem', letterSpacing: '3px', marginBottom: '16px' };
const archiveHeaderBlock = { minWidth: 0, maxWidth: '760px', alignSelf: 'center' };
const archiveHeaderBlockTextOnly = { ...archiveHeaderBlock, maxWidth: '900px', width: '100%', padding: '0 4px' };
const archiveTitle = { fontFamily: '"Noto Serif TC", "Songti TC", "PMingLiU", "Cinzel", serif', fontSize: 'clamp(1.85rem, 2.85vw, 2.95rem)', fontWeight: 400, lineHeight: 1.18, letterSpacing: '2px', margin: '14px 0 0', maxWidth: '920px', color: 'rgba(245,238,222,0.92)', textShadow: '0 0 18px rgba(188,19,254,0.10)', whiteSpace: 'pre-line' };
const ornamentDivider = { display: 'grid', gridTemplateColumns: '1fr auto 1fr', alignItems: 'center', gap: '12px', width: '100%', maxWidth: '520px', margin: '22px 0 20px', color: '#bc13fe' };
const archiveLead = { fontFamily: '"Noto Serif TC", "Microsoft JhengHei", serif', fontSize: '1rem', lineHeight: 1.9, letterSpacing: '0.8px', color: 'rgba(255,255,255,0.72)', maxWidth: '760px', margin: 0 };
const archiveBodyPanel = { marginTop: '32px', padding: '28px 34px', borderTop: '1px solid rgba(212,175,55,0.18)', borderBottom: '1px solid rgba(212,175,55,0.12)', background: 'linear-gradient(180deg, rgba(255,255,255,0.025), rgba(188,19,254,0.025))', boxShadow: 'inset 0 0 28px rgba(188,19,254,0.035)' };
const archiveParagraph = { fontFamily: '"Noto Serif TC", "Microsoft JhengHei", serif', fontSize: '1.05rem', lineHeight: 2.05, letterSpacing: '0.8px', color: 'rgba(255,255,255,0.76)', margin: '0 0 18px' };
const archiveFirstParagraph = { ...archiveParagraph, fontSize: '1.08rem', color: 'rgba(255,255,255,0.82)' };
